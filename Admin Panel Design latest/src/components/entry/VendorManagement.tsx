...
export function VendorManagement() {
  ...
  const fetchVendors = async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/core-vendors', { withCredentials: true });
      if (res.data.success) {
  ...
  const handleAddVendor = async () => {
    ...
    try {
      ...
      await axios.post('http://localhost:5000/api/core-vendors', payload, { withCredentials: true });
  ...
  const handleDeleteVendor = async (id: string) => {
    try {
      await axios.delete(`http://localhost:5000/api/core-vendors/${id}`, { withCredentials: true });
