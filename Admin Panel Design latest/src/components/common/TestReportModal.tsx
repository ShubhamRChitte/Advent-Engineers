...
export function TestReportModal({ isOpen, onClose, transformer, order, testType }: TestReportModalProps) {
    ...
    const fetchReportDetail = async () => {
        ...
        try {
            ...
            const res = await fetch(`http://localhost:5001/api/reports/${id}?stage=${testType}`);
    ...
    const fetchCoreData = async () => {
        ...
        try {
            ...
            // Fetch Metering
            const metRes = await fetch(`http://localhost:5001/api/metering-tests/${orderId}`);
            const metData = await metRes.json();
            if (metData) setMeteringData(metData);

            // Fetch Protection
            const protRes = await fetch(`http://localhost:5001/api/protection-tests/${orderId}?type=Protection`);
            const protData = await protRes.json();
            if (protData) setProtectionData(protData);

            // Fetch PS
            const psRes = await fetch(`http://localhost:5001/api/protection-tests/${orderId}?type=PS`);
