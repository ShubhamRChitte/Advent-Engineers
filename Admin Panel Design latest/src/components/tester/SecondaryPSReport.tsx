...
export function SecondaryPSReport({ transformer, coreNumber, coreId, testerName, onBack, readOnly = false }: any) {
  ...
  // Initialize with at least one row or based on transformer ratios if available
  // Assuming transformer might have a 'ratios' array or similar, but for now we default to a standard set 
  // OR we try to pull from `transformer.ratios` (if standard format).
  // Let's use a standard template or what was passed.
  const initialRatios = transformer.ratios && transformer.ratios.length > 0 ? transformer.ratios : ['200/1', '400/1', '800/1'];
...
<<<<<<< HEAD
  ...
  React.useEffect(() => {
    const loadData = async () => {
      try {
        // Check if we have data in the passed transformer object first (prop drilling optimization)
        ...
        const existingResults = fresh.testHistory?.secondary_test?.ps_results;

        if (existingResults && existingResults.length > 0) {
          // Filter for this core
          const myResults = existingResults.filter((r: any) => r.internalCoreNo === coreId || r.coreId === coreId);

          if (myResults.length > 0) {
            setPsData(prev => prev.map(row => {
              const saved = myResults.find((s: any) => s.ratioValue === row.ratioValue);
              if (saved) {
                return { ...row, ...saved, vkVal: saved.vkVal || saved.iex11Vk /* fallback? no, vkVal is 1.1Vk */ };
                // Wait, schema has vkVal, iexVk, iex11Vk.
              }
              return row;
            }));
          }
        }
      } catch (e) { console.error("Error loading PS data", e); }
    };
    loadData();
...
<<<<<<< HEAD
    try {
      ...
      await axios.post('http://localhost:3002/api/secondary-ps-tests', payload, { withCredentials: true });
      toast.success("PS Test Data Saved!");
    } catch (err: any) {
...
<<<<<<< HEAD
                    <td className="border border-gray-400 p-1 bg-white border-b-0 h-8 flex items-center">
                      ...
                      <Input
                        className="border-none text-[#0070c0] font-bold h-6 shadow-none flex-1 disabled:opacity-100 disabled:cursor-not-allowed"
                        value={row.vk}
