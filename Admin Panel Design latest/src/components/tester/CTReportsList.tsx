...
interface CTReportsListProps {
    onBack: () => void;
}

export function CTReportsList({ onBack }: CTReportsListProps) {
    // Data State
    ...
    const fetchReports = async () => {
        ...
        try {
            ...
            const response = await axios.get('http://localhost:5001/api/final/reports', {
    ...
    if (selectedTransformer) {
        ...
        const finalTest = selectedTransformer.testHistory?.final_test || {};
    ...
    const getGroupLatestDate = (transformers: CompletedTransformer[]): number => {
        const dates = transformers.map(t => {
            const finalDateStr = t.testHistory?.final_test?.timestamp || t.testHistory?.final_test?.date;
            if (!finalDateStr) return 0;
            const parts = typeof finalDateStr === 'string' ? finalDateStr.split('/') : [];
        ...
        <div className="space-y-6">
            <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-3 w-full">
                    {isAdmin && (
                        <select
                            className="px-3 py-2 bg-white border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-[150px] w-48"
            ...
            {loading ? (
                ...
                <Card className="p-12 border-dashed border-2 border-gray-200 bg-gray-50/50 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-gray-500 max-w-sm mt-2">
                        You haven't completed and approved any CT tests yet.
                    </p>
                </Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredJobIds.map(jobId => {
                        ...
                        return (
                            <Card
                                key={jobId}
                                className="group hover:shadow-lg transition-all duration-200 border-gray-200 hover:border-blue-300 overflow-hidden"
                            >
                                <div className="p-5 space-y-4">
                                    ...
                                    <div className="pt-2 flex items-center justify-end border-t border-gray-100">
                                        <button
                                            className="text-xs font-semibold text-blue-600 flex items-center hover:translate-x-1 transition-transform"
