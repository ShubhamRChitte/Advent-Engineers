...
const FailedCoresPage = lazy(() => import('../../pages/FailedCoresPage'));
...
export function TesterLayout({ user, onLogout }: TesterLayoutProps) {
  ...
  const renderView = () => {
    // Shared Views
    if (activeView === 'failed-cores') {
      return (
        <Suspense fallback={<div className="flex h-96 items-center justify-center text-gray-500">Loading Dashboard...</div>}>
          <FailedCoresPage />
        </Suspense>
      );
    }
    ...
    if (user.role === 'secondary-tester') {
      if (activeView === 'home') {
        ...
        return <SecondaryReportsList onBack={() => setActiveView('home')} onViewReport={() => { }} />;
      }
    }
