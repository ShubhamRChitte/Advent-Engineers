...
export function PTReportsList({ onBack }: PTReportsListProps) {
    ...
    const fetchReports = async () => {
        setLoading(true);
        try {
            const response = await axios.get('http://localhost:3002/api/pt-tests/reports', {
            ...
            {loading ? (
                ...
                <Card className="p-12 border-dashed border-2 border-gray-200 bg-gray-50/50 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-gray-500 max-w-sm mt-2">
                        ...
                        Once you save a test in the "Testing" tab, it will appear here.
                    </p>
                </Card>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {jobIds.map(jobId => {
                        ...
                        // Find most recent test date in this group (from PT testing date strings)
                        // Note: Depending on string format, we might just grab the first one if sort is complex
                        const lastTestDate = transformers[0]?.testHistory?.pt_test?.date || 'N/A';
                                ...
                                <div className="p-5 space-y-4">
                                    ...
                                    <div className="space-y-2 pt-2 border-t border-gray-100">
                                        <div className="flex items-center text-sm text-gray-600">
                                            ...
                                            <span>Test Date: {lastTestDate}</span>
                                        </div>
                                        <div className="flex items-center text-sm text-gray-600">
