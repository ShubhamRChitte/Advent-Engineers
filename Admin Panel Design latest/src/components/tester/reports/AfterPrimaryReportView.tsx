...
interface AfterPrimaryReportViewProps {
    transformer: any;
    onBack: () => void;
}

export function AfterPrimaryReportView({ transformer, onBack }: AfterPrimaryReportViewProps) {
    ...
    const getReportType = () => {
        ...
        if (transformer.testHistory?.primary_test?.metering_results?.length > 0) return 'metering';
        if (transformer.testHistory?.primary_test?.protection_results?.length > 0) return 'protection';
        if (transformer.testHistory?.primary_test?.ps_results?.length > 0) return 'ps';
    ...
    // Logic to determine which reports to show based on history (primary_test)
            ...
            <div className="space-y-8">
                {(!hasHistory || showMetering) && (reportType === 'metering' || hasHistory) && (
                    <SecondaryMeteringReport
                        ...
                        testerName={transformer.testHistory?.primary_test?.tester || 'Unknown'}
