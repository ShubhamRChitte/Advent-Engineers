...
export function HeatingTransformersList({ order, onStartTest, onBack }: HeatingTransformersListProps) {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const resTrans = await axios.get(`http://localhost:5000/api/heating-record/transformers/${order._id}`, {
  ...
  const handleStartHeating = async (transformer: Transformer) => {
    ...
    if (transformer.status === 'pending') {
      try {
        await axios.put(`http://localhost:5000/api/heating-record/start/${transformer.uniqueId}`, {}, { withCredentials: true });
  ...
  const handleApprove = async (transformer: Transformer) => {
      try {
          await axios.post(`http://localhost:5000/api/heating-record/save/${transformer.uniqueId}`, {
              isApproveCall: true
          }, { withCredentials: true });
