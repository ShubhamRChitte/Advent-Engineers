...
export function TesterNotifications({ userRole, onViewOrder, setActiveView }: TesterNotificationsProps) {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const res = await axios.get(`/notifications`, { withCredentials: true, headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
  ...
  const handleMarkAsRead = async (notificationId: string) => {
    try {
      await axios.put(`/notifications/${notificationId}/read`, {}, { withCredentials: true, headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
