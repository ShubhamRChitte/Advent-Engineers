...
export interface CoreTestingOrder {
  id: string;
  _id?: string;
  orderId?: string | { _id: string }; // Handle populated vs unpopulated
  mainOrderId?: string;
  jobId: string;
  clientName: string;
  ...
  rating: string;
  coreDetails: any[];
  assignedUnitIds?: string[]; // Granular visibility
  order?: any; // Fallback
...
export function CoreTestingForm({ order, coreType, onBack, isReadOnly = false, user }: CoreTestingFormProps) {
  ...
  const getNextSequenceNumber = (currentRows: CoreTestRow[]) => {
    // ... existing getNextSequenceNumber logic ... (omitted for brevity, assume unchanged unless specified)
  ...
  useEffect(() => {
    const loadExistingData = async () => {
      ...
      try {
        ...
        // Fetch existing data relative to BaseURL
        // Note: These endpoints likely also need to be centralized eventually, 
        // but focusing on failed-cores for now as per request.
        const response = await apiClient.get(`${endpoint}/${txnOrderId}${typeParam}`);
  ...
  const calculateRemark = (row: CoreTestRow): string => {
    // ... existing calculate remark logic ...
    if (isProtectionCore) {
  ...
  const handleReplaceCore = async (index: number) => {
    if (isReadOnly) return;
    const failedRow = rows[index];
    const systemDate = getSystemDate();

    // SAFE ORDER ID ACCESS (Same as handleSave)
    // Safe narrowing for order items
    const txnOrderId = (order as any).mainOrderId
      || ((typeof (order as any).orderId === 'object' && (order as any).orderId !== null) ? (order as any).orderId._id : (order as any).orderId)
      || (order as any)._id
      || order.id;

    if (!txnOrderId) {
      alert("Error: Could not identify Order ID. Cannot record failure.");
      return;
    }

    // Archive the failure to the FailedCores state
    const failedCore: FailedCore = {
      orderId: txnOrderId, // Use safe string ID
      jobId: order.jobId,
    ...
    // --- BACKEND API CALL (Transaction Safe) ---
    try {
      // Use configured API client and centralized route
      await apiClient.post(API_ROUTES.FAILED_CORES, {
        orderId: txnOrderId,
        internalCoreNo: failedRow.internalCoreNo,
        vendorCoreNo: failedRow.coreVendorNo,
        coreType: coreType,
        vendorName: "Unknown Vendor", // Ideally fetch from Vendor API or Dropdown
        failureReason: getFailureReason(failedRow) || "Manual Replacement",
        failureStage: "INITIAL_TEST",
        dynamicValues: failedRow.dynamicValues
      });
      // Success notification is optional/implicit via UI update
    } catch (error) {
      console.error("Failed to record failure in backend:", error);
      alert("Warning: Failed to save failure record to server. Please verify connection.");
      // We continue with UI update to not block work, but this is critical.
    }
    ...
    updatedRows[index] = {
      date: systemDate,
      coreVendorNo: '',
      internalCoreNo: generateCoreId(Number(nextSeq) || 0),
      value1000: '', value3000: '', value5000: '', value7000: '',
      singleValue: '',
      dynamicValues: {},
      remark: '',
  ...
  const addRow = () => {
    ...
    setRows([...rows, {
      date: getSystemDate(),
      coreVendorNo: '',
      internalCoreNo: generateCoreId(Number(nextSeq) || 0), // Fixed naming here
      value1000: '',
      value3000: '',
      value5000: '',
      value7000: '',
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      // Send Batch Approval
      const response = await apiClient.put(`/core-tests/approve-batch`, {
        jobId: order.jobId,
        internalCoreNos
      });
  ...
  if (showPrintLabels) {
    return (
      <CoreLabelsPrint
        ...
        order={{
          ...order,
          orderId: ((typeof order.orderId === 'object' && order.orderId !== null)
            ? (order.orderId as any)._id
