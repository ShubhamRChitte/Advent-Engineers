...
export function CoreTypeSelection({ order, onSelectCoreType, onBack }: CoreTypeSelectionProps) {
    ...
    const fetchCompletionStatus = async () => {
      ...
      try {
        await Promise.all(uniqueTypes.map(async (type) => {
          ...
          try {
            const res = await axios.get(`http://localhost:5001/api${endpoint}/${txnOrderId}${typeParam}`, {
  ...
  const handleGlobalApprove = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:5001/api/core-tests/approve/${txnOrderId}`, {}, { withCredentials: true });
