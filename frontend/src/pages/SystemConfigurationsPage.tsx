...
export function SystemConfigurationsPage() {
  ...
  const [idCategoryTab, setIdCategoryTab] = useState<'orderId' | 'transformerId' | 'preTestBatchId' | 'preTestCoreId'>('orderId');
    ...
    <div className="space-y-6 w-full pb-8">
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
        ...
        <TabsContent value="system" className="mt-6 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
            <div>
              <h3 className="text-xl font-extrabold text-gray-900 flex items-center gap-2">
                ...
                Global Flowchart ID Builder
              </h3>
              <p className="text-xs text-gray-500 mt-1">
                Customize dynamic ID structures, add year/month/counter fields, and control auto-increment rules.
              </p>
            </div>

            <Button onClick={saveIdSettings} disabled={saving} className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-6 shadow-md shrink-0">
              <Save className="w-4 h-4 mr-2" />
              {saving ? 'Saving...' : 'Save All ID Configurations'}
            </Button>
          </div>
          ...
          {loading ? (
            <div className="text-gray-500 py-12 text-center bg-white rounded-2xl border border-gray-200">
              Loading ID settings...
            </div>
          ) : (
            <div>
