...
interface AfterPrimaryCompletedTransformersListProps {
...
export function AfterPrimaryCompletedTransformersList({
    transformers,
    onViewReport,
    onBack,
    jobId,
    clientName
}: AfterPrimaryCompletedTransformersListProps) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                <h2 className="text-2xl font-bold text-gray-800">Completed Reports (After Primary) - {jobId}</h2>
                                ...
                                {transformers.map((t) => (
                                    <tr key={t._id} className="hover:bg-blue-50/50 transition-colors">
                                        ...
                                        <td className="p-4 text-gray-600">
                                            {t.testHistory?.primary_test?.timestamp
                                                ? new Date(t.testHistory.primary_test.timestamp).toLocaleString()
                                                : 'N/A'}
                                        </td>
                                        <td className="p-4 text-gray-600">
                                            <div className="flex items-center gap-2">
                                                <span className="bg-gray-100 text-gray-700 px-2 py-0.5 rounded text-xs border border-gray-200">
                                                    {t.testHistory?.primary_test?.tester || 'Unknown'}
                                                </span>
                                            </div>
                                        </td>
