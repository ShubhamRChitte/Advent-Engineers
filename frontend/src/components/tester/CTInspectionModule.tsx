...
export function CTInspectionModule({ userName }: CTInspectionModuleProps) {
  ...
  const fetchBatches = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/ct-inspection/batches', { withCredentials: true });
  ...
  if (openReport && selectedBatch) {
    ...
    return (
      <div>
        ...
        <CTInspectionReport
          transformer={{ uniqueId: openReport.transformerId, ...batchTransformer }}
          testerName={userName || 'Tester'}
          batchId={openReport.batchId}
          batchTransformerId={openReport.transformerId}
          onSaved={async () => {
            setOpenReport(null);
            try {
              const res = await axios.get('/ct-inspection/batches', { withCredentials: true });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">CT Inspection Module</h2>
          <p className="text-gray-500 text-sm mt-0.5">Manage inspection batches for completed CT transformers</p>
        </div>
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'center' }}>
            <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#374151', margin: 0 }}>
              No CT Inspection Batches Yet
            </h3>
            <p style={{ fontSize: '14px', color: '#6b7280', marginTop: '6px', maxWidth: '320px' }}>
              Create your first inspection batch by selecting completed CT transformers from any job.
            </p>
          </div>
          <button
            onClick={() => setShowCreateModal(true)}
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ height: '5px', background: '#f3f4f6', borderRadius: '9999px', overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${pct}%`, background: '#6366f1', borderRadius: '9999px' }} />
                  </div>
                </div>
              </div>
            );
