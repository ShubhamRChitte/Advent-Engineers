...
interface PTPretestReportProps {
  ...
  onBack: () => void;
  user: any; // The logged-in PT Testing Engineer user
...
export function PTPretestReport({ order, transformer, onBack, user, noTimer = false, onApproveSuccess }: PTPretestReportProps) {
  ...
  const { timeLeftMs, isOverdue, expectedMinutes, endTimer } = usePTTimer({
    ...
    jobId:         order?.jobId || '',
    stage:         'pt_pretest',
    testerName:    user?.name || user?.fullName || 'PT Testing Engineer',
    role:          'pt-pretester',
            ...
            for (const t of responseList) {
                ...
                if (testRes.data.success && testRes.data.data) {
                    ...
                    if (!isReadOnly && !savedData.testedBy) {
                        savedData.testedBy = user?.name || user?.fullName || 'Testing Engineer';
                    }
                    ...
                    newReportsData[t._id] = {
                        ...
                        testedBy: user?.name || user?.fullName || 'Testing Engineer',
                        signature: '',
  ...
  const handleLogFailure = async () => {
    ...
    try {
        const payloads = failingCoresInfo.map(info => ({
            ...
            reportedBy: user?.name || user?.fullName || 'PT Testing Engineer'
        }));
                ...
                {transformersData.map((transformer) => {
                    ...
                    return (
                        <PTPretestPrintableReport
                            ...
                            onChange={(core, field, val) => handleInputChange(transformer._id, 'preTesting', core, val, field)}
                            onTestedByChange={(val) => handleInputChange(transformer._id, '', 'testedBy', val)}
                        />
                    );
                })}
            </div>
